...
// register invite redemption module (must be above the catch-all /:noteId route)
appRouter.use(require('./invite'))

// register admin panel (must be above the catch-all /:noteId route)
appRouter.use(require('./admin'))

// register personal notes dashboard API (must be above the catch-all /:noteId route)
appRouter.use(require('./dashboard').router)

// register shared spaces & browse API (must be above the catch-all /:noteId route)
appRouter.use(require('./browse').router)

// register copy & templates API (must be above the catch-all /:noteId route)
appRouter.use(require('./teach').router)

// register inline comments API (must be above the catch-all /:noteId route)
appRouter.use(require('./comment').router)
