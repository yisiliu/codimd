...
// what makes the last-owner guard atomic.
function rowLock (t) {
  return models.sequelize.getDialect() === 'sqlite' ? undefined : t.LOCK.UPDATE
}

// Atomic last-owner guard. Lock the OTHER active owners' rows inside the
// transaction so a concurrent deactivate/demote can't also observe "another owner
...
// (appRouter.use(require('./admin'))), so a bare router.use(requireAdmin) would
// run for EVERY request that reaches it — blocking /me, /new, notes, etc. for
// non-admins. Restricting the path keeps requireAdmin on admin routes alone;
