...
// append one space to a note (bulk "add to space") — only if the owner is a member
async function addNoteToSpace (ownerId, noteId, spaceId) {
  await ownedNote(ownerId, noteId)
  const mySpaces = await memberSpaceIds(ownerId)
  if (!mySpaces.map(String).includes(String(spaceId))) throw new Error('space-not-found')
  await models.NoteSpace.findOrCreate({ where: { noteId, spaceId }, defaults: { noteId, spaceId } })
}
...
async function listBrowse (user, { space } = {}) {
  ...
  const openComments = {}
  const crows = await models.Comment.findAll({ where: { noteId: notes.map(n => n.id), parentId: null, resolved: false }, attributes: ['noteId'] })
  crows.forEach(r => { openComments[r.noteId] = (openComments[r.noteId] || 0) + 1 })
...
Object.assign(module.exports, { setNoteSpaces, addNoteToSpace, spacesForNote, listBrowse, searchBrowse, ownedNote, listMembers, addMember, removeMember, transferSteward, listUsers })
