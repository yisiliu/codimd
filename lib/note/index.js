...
const { Note, User, Revision, NoteTag, NoteSpace, Comment } = require('../models')
const browse = require('../browse')

const { newCheckViewPermission, errorForbidden, responseCodiMD, errorNotFound, errorInternalError } = require('../response')
const { updateHistory, historyDelete, getHistory } = require('../history')
...
// map of noteId -> number of unresolved TOP-LEVEL comments (replies excluded)
async function unresolvedCommentCounts (noteIds) {
  const out = {}
  if (!noteIds || !noteIds.length) return out
  const rows = await Comment.findAll({ where: { noteId: noteIds, parentId: null, resolved: false }, attributes: ['noteId'] })
  rows.forEach(r => { out[r.noteId] = (out[r.noteId] || 0) + 1 })
  return out
}
  ...
  try {
    ...
    const myNoteList = await Promise.all(myNotes.map(async note => {
      ...
      return {
        ...
        commentCount: openComments[note.id] || 0,
...
async function cleanupNoteOrganization (noteId) {
  ...
  await Comment.destroy({ where: { noteId } })
}
