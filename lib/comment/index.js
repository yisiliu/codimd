...
async function listComments (user, noteId) {
  ...
  // top-level comments ordered by line/time, then replies ordered by time
...
async function addComment (user, noteId, { line, anchorText, content, parentId }) {
  const note = await loadNote(noteId)
  if (!canView(user, note)) throw new Error('forbidden')
  const text = (content || '').trim()
  if (!text) throw new Error('comment content required')
  if (text.length > 2000) throw new Error('comment too long')
  // a reply inherits the parent's line/anchor; only one level of nesting is allowed
  if (parentId) {
    const parent = await loadComment(parentId)
    if (String(parent.noteId) !== String(noteId)) throw new Error('parent-not-found')
    if (parent.parentId) throw new Error('cannot reply to a reply')
    return models.Comment.create({ noteId, authorId: user.id, parentId: parent.id, line: parent.line, anchorText: parent.anchorText, content: text })
  }
...
async function deleteComment (user, commentId) {
  ...
  const note = await loadNote(c.noteId)
  if (!canModerate(user, note, c)) throw new Error('forbidden')
  // deleting a top-level comment removes its whole thread
  if (!c.parentId) await models.Comment.destroy({ where: { parentId: c.id } })
  await c.destroy()
}
